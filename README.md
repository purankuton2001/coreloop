...
coreloop が「掘る→言語化→シェア」のループを扱うのに対し、[`packages/keeploop`](packages/keeploop) は
「また明日も来る」ループ — ストリーク（休息日つき）・上限つき付与・XP とレベル・同点を正直に扱う
ランキング・週次リーグ（昇降格）・1 日 1 通のナッジ判定 — を扱います。
別の npm パッケージ（`keeploop`）で、依存ゼロ。文言・保存・配送は同じく持ちません。
