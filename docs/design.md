...
1. **プロダクト固有の言葉を持たない** — そのプロダクトの質問文・評価軸・語彙・コピーは
   同梱しない。外販時に「掘り方＝商品」を守る線でもある。
   **例外は公開フレームワークの標準的な問い**（`coreloop/frameworks`、§3.9）: 誰の資産でもなく
   全アプリが同じ出典から書き写していたものなので、差し替え可能な既定値として持つ
...
coreloop/frameworks  ライフチャート / 9マス / ジョハリ / 円の重なり / 他者視点
                     ＋ 各フレームワークの標準の問い（framework-questions.ts、差し替え可）
```
...
### 3.9 公開された自己分析フレームワークは形と標準の問いを持つ
...
**問いも既定として入っている**（`framework-questions.ts`）。原則1が守っているのは
「そのプロダクトの掘り方」であって、「人生を線で描いてください」は誰の資産でもない —
各アプリが同じ出典から書き写す作業を残す理由がない。ただし:

- 計算するファイル（`frameworks.ts`）には文言を1語も置かない。コピーは別ファイルに隔離し、
  entry point で再 export するだけ。自前の声を持つプロダクトは丸ごと捨てられる
- テンプレートは ja/en の `LocalizedText` ＋ `{{placeholder}}`。既存の `fillTemplate` と同じ規約で、
  値が無い placeholder は空にせず残す（半端な問いが人に届く前にログで見える）
- signal がそのまま変数になる（turning point の `at`、空きマスの `count`、blind の `traits`）
...
### 3.10 背景情報は出典と予算を保つ（`coreloop/context`）

CORECORD の過去の会話・作品と、PROTAGONIST の行動記録・前回の物語では、
本人の発言・保存記録と生成された文章を区別して次の生成へ渡す必要がある。
共通化するのは、この既存の素材を出典付きで並べ、予算内に組み立てる部分だけ。
PROTAGONIST への新しい記憶機能の導入や、両アプリ間の情報共有は含まない。

`ContextItem` はスコープ、出典、由来、本文を持つ。`buildContext` はスコープ完全一致を
確認し、同一 item の重複を除き、pinned → 呼び出し側の順序で、JSON 全体の文字数に収める。
出典だけ、本文の途中だけを落とすことはない。pinned の総量が予算を超えたら型付きの例外にする。
基本プロフィールが検索結果から落ちる問題を、意味検索の順位とは独立に扱える。

`recordedAt` と `eventAt` は別の任意文字列。生成された物語に記録日があっても、
その出来事がその日に起きたとは解釈しない。由来も事実判定の代用ではない。
作品や解釈を本人の経歴へ昇格しない指示、原文の所有者・削除状態の確認、訂正の反映、
関連度の選択はアプリ側の責務。scope の照合は認可への追加防御であり、認可そのものではない。
